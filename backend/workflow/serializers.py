...
class RequestSubjectSerializer(serializers.ModelSerializer):
    class Meta:
        model = RequestSubject
        fields = ['id', 'name', 'code', 'workflow']
...
class WorkflowConfigSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'name', 'category', 'description', 'steps', 'subjects']

    def get_subjects(self, obj):
        """Return only active subjects for this workflow."""
        active_subjects = obj.subjects.filter(is_active=True)
        return RequestSubjectSerializer(active_subjects, many=True).data
...
class RequestSerializer(serializers.ModelSerializer):
    ...
    # reqSubject: write with list of IDs, read back as full objects via reqSubject_details
    reqSubject = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=RequestSubject.objects.filter(is_active=True),
        required=False
    )
    reqSubject_details = serializers.SerializerMethodField(read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            # IT request fields
            'reqSubject', 'reqSubject_details', 'priorify', 'expectDate', 'auditFlag',
        ]
    ...
    def get_reqSubject_details(self, obj):
        return [
            {'id': s.id, 'name': s.name, 'code': s.code}
            for s in obj.reqSubject.all()
        ]

    def create(self, validated_data):
        uploaded_files = validated_data.pop('uploaded_files', [])
        req_subjects = validated_data.pop('reqSubject', [])
        request_obj = Request.objects.create(**validated_data)
        if req_subjects:
            request_obj.reqSubject.set(req_subjects)
    ...
    def update(self, instance, validated_data):
        uploaded_files = validated_data.pop('uploaded_files', [])
        req_subjects = validated_data.pop('reqSubject', None)
        instance = super().update(instance, validated_data)
        if req_subjects is not None:
            instance.reqSubject.set(req_subjects)
        for file in uploaded_files:
            RequestFile.objects.create(request=instance, file=file)
        return instance
