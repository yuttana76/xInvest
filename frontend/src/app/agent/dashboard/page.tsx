...
import { getAgentDashboardStats, type AgentDashboardStats } from '@/lib/api/investor';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip } from 'recharts';

const COLORS = ['#10b981', '#3b82f6', '#f59e0b', '#8b5cf6', '#ec4899', '#06b6d4'];

export default function AgentDashboard() {
  const [data, setData] = useState<AgentDashboardStats | null>(null);
  ...
  useEffect(() => {
    ...
    getAgentDashboardStats().then(setData).finally(() => setLoading(false));
  }, []);
  ...
  const pieData = statusDistribution ? Object.entries(statusDistribution).map(([name, value]) => ({ name, value })) : [];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[
          { label: "Total referred AUM", value: `$${stats.totalAUM.toLocaleString()}`, icon: <Wallet size={20} />, color: "emerald" },
          { label: "Total Referred Investors", value: stats.totalInvestors, icon: <Users size={20} />, color: "blue" },
        ].map((stat, i) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            { 
              ...
              description: 'Private wealth solutions'
            },
          ].map((product, i) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass p-8 rounded-2xl border border-white/5">
          ...
          <div className="space-y-4">
            {pieData.map((item, i) => {
                const percent = stats.totalInvestors > 0 ? (item.value / stats.totalInvestors * 100).toFixed(0) : '0';
                return (
                    <div key={i}>
