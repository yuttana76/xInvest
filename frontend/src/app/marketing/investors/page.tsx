...
import { getMarketingInvestorList, type MarketingInvestor } from '@/lib/api/investor';
import { cn } from '@/utils/cn';

export default function MarketingInvestors() {
  ...
  const fetchInvestors = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getMarketingInvestorList(status, search);
      setInvestors(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Assigned Investors</h1>
          <p className="text-slate-400 mt-1">Manage and monitor investors in your portfolio.</p>
        </div>
