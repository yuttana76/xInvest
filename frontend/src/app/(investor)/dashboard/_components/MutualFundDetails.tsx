...
import { ChevronUp, ChevronDown, ThumbsUp, ThumbsDown, TrendingUp } from 'lucide-react';
    ...
    <div className="bg-black/20 rounded-2xl border border-white/5 p-6 space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xl font-bold text-slate-100">MF Details</h3>
        </div>
        <div className="flex items-center gap-4 ml-auto text-right flex-wrap sm:flex-nowrap justify-end">
          {accountIds.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <select 
                className="bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-xs outline-none text-slate-100 max-w-[120px] text-ellipsis"
                value={innerSelectedAccount}
                onChange={(e) => setInnerSelectedAccount(e.target.value)}
              >
                <option value="all" className="bg-slate-800">All</option>
                {accountIds.map(acc => (
                  <option key={String(acc)} value={String(acc)} className="bg-slate-800">{String(acc)}</option>
                ))}
              </select>
            </div>
          )}
          <div className="flex flex-col items-end gap-1">
            ...
            <p className="text-xs font-medium text-slate-400">
              Profit: <span className={totalProfit >= 0 ? 'text-emerald-400' : 'text-rose-500'}>
                {totalProfit >= 0 ? '+' : ''}{new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(totalProfit)}
                </span>
            </p>
            <p className="text-xs font-medium text-slate-400">
                <span className={totalProfit >= 0 ? 'text-emerald-400' : 'text-rose-500'}>
      ...
      <div className="w-full">
        <div className="flex items-center justify-between mb-4">
          <h4 className="text-sm font-semibold text-slate-200">Portfolio Performance</h4>
      ...
      <div className="space-y-4">
        <h4 className="text-sm font-semibold text-slate-200">Holdings ({filteredBalances.length})</h4>
                    ...
                    >
                      ...
                      {hoveredSentimentKey === holdingKey && b.fund_analysis && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-64 z-50 animate-in fade-in zoom-in-95 duration-200 pointer-events-none">
                          <div className="backdrop-blur-xl border border-white/10 p-4 rounded-2xl shadow-2xl relative bg-slate-900/90">
                            ...
                            <div className="flex items-center gap-2 mb-2 pb-2 border-b border-white/5">
                              ...
                              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Market Sentiment</span>
                            </div>
                  ...
                  <div className="text-right shrink-0">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <div className={`text-xs font-bold ${profit >= 0 ? 'text-emerald-400' : 'text-rose-500'}`}>
                          {profit >= 0 ? '+' : ''}{new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(profit)}
                        </div>
                        <div className={`text-[10px] font-medium ${profit >= 0 ? 'text-emerald-400/80' : 'text-rose-500/80'}`}>
                ...
                {isExpanded && (
                  <div className="animate-in slide-in-from-top-1 duration-200">
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-xs pl-2 pt-2">
                      <div className="text-slate-400">Unit Balance: <span className="text-slate-100 block font-medium mt-0.5">{Number(b.unitBalance).toLocaleString(undefined, { minimumFractionDigits: 4, maximumFractionDigits: 4 })}</span></div>
                      <div className="text-slate-400">Average Cost: <span className="text-slate-100 block font-medium mt-0.5">{Number(b.averageCost).toFixed(4)}</span></div>
                      <div className="text-slate-400">NAV: <span className="text-slate-100 block font-medium mt-0.5">{Number(b.NAV).toFixed(4)}</span></div>
                      <div className="text-slate-400">NAV Date: <span className="text-slate-100 block font-medium mt-0.5">{new Date(b.NAVdate).toLocaleDateString('en-GB')}</span></div>
                    </div>
                  </div>
                )}
              </div>
